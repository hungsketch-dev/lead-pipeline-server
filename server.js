...
// Privy OAuth token exchange
app.post('/privy-token', async (req, res) => {
  try {
    const { client_id, client_secret, scope } = req.body;
    const params = new URLSearchParams({ grant_type: 'client_credentials', client_id, client_secret });
    if (scope) params.append('scope', scope);
    const r = await fetch('https://api.privy.com/oauth/token', {
      method: 'POST',
...
// Privy create contact
app.post('/privy-contact', async (req, res) => {
  try {
    const { token, contact } = req.body;
    const r = await fetch('https://api.privy.com/v1/contacts', {
      method: 'POST',
